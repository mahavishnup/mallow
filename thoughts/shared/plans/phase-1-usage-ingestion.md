...
- [ ] Columns: `id`, `uuid` nullable uuid **unique** (alias for idempotency key; keep column name `uuid`), `merchant_id` FK, `customer_id` FK, `usage_date date`, `quantity unsignedBigInteger`, `type string`, `aggregated_at timestamp nullable`, timestamps.
- [ ] Indexes: unique `(uuid)`; `(customer_id, usage_date)`; `(merchant_id, usage_date)`; partial index on `(aggregated_at)` **`WHERE aggregated_at IS NULL`** — sqlite supports partial indexes, keep syntax portable.
- [ ] Verify partial index SQL runs on sqlite (test) and local DB.

### 2.2 API plumbing

- [ ] Create `routes/api.php` (empty group to start). Register in `bootstrap/app.php`: `->withRouting(web: …, api: __DIR__.'/../routes/api.php', commands: …, health: …)` (C0.9).
...
- [ ] `app/Console/Commands/CreateApiKeyCommand.php` (`billing:api-key {--merchant=} {--name=}`): creates key, prints **plaintext once** (`mall_` prefix + random 32 bytes, hex). Never store plaintext.
...
- [ ] Validate → action → respond. `201 {data: {…}}` on stored; `200 {duplicate: true}` on duplicate (Plan §5.1). Never 4xx on duplicate — client retries must be safe.
- 429 and 401 come from middleware/throttle; no controller logic needed.

### 2.6 Tests — `tests/Feature/Api/RecordUsageTest.php` (Pest)

Cover Plan §10 "API / Platform" rows:

- [ ] valid request → 201, event row exists with `aggregated_at = null`
- [ ] identical retry (same `idempotency_key`) → 200 `{duplicate:true}`, still **1 row**
- [ ] missing/invalid `X-Api-Key` → 401
- [ ] validation failures (bad date, qty 0/negative/string, missing key) → 422
- [ ] usage_date in the future → 422
- [ ] tenant isolation: merchant B's key + merchant A's customer → 422/404, no row
- [ ] customer without active subscription → 422
- [ ] rate limit: exceed merchant bucket → 429 (use `RateLimiter::clear` between tests; small custom config override in test to e.g. 2/min for speed)
- [ ] per-customer limit independent of merchant limit

## 3. Acceptance Criteria

- [ ] `php artisan route:list --path=api` shows `POST api/usage` with both throttles + key middleware.
- [ ] Duplicate submission never creates a second row (DB-level proof: attempt direct duplicate insert in a test → constraint violation caught → `duplicate:true`).
- [ ] 401/422/429 paths tested. Tenant isolation proven.
