...
return [
    ...
    'description' => 'Multi-tenant subscription billing & usage-metering API. Every request is scoped to a merchant (tenant) resolved from its API key.',

    // Text to place in the "Introduction" section, right after the `description`. Markdown and HTML are supported.
    'intro_text' => <<<'INTRO'
        ## Tenancy (merchant scoping)
    ...
    'auth' => [
        // All /api/* endpoints (except the dashboard's session fallback) authenticate by tenant API key.
        'enabled' => true,

        // Authenticated by default; the dashboard endpoint can note its session fallback via @unauthenticated-style docs.
        'default' => true,

        // The API key is sent in a request header (X-Api-Key), matching ResolveApiKey middleware.
        'in' => AuthIn::HEADER->value,

        // The header Scribe will show and use in example requests.
        'name' => 'X-Api-Key',
        ...
        'extra_info' => 'API keys are **tenant-scoped**: each key maps to exactly one merchant, and that merchant is the '
            . 'tenant for the request. Generate one with `php artisan billing:api-key --merchant={id}`. '
            . 'The dashboard endpoint additionally accepts an authenticated team-member session.',
    ],
